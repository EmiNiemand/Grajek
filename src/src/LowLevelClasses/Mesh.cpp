...
void Mesh::Draw(Shader *shader, int type) {
...
void Mesh::DrawInstanced(Shader* shader, int type, int amount)
{
    // bind appropriate textures
    unsigned int diffuseNr  = 1;
    unsigned int specularNr = 1;
    ...
    // draw mesh
    glBindVertexArray(vao);
    glDrawElementsInstanced(type, static_cast<unsigned int>(indices.size()), GL_UNSIGNED_INT, 0, amount);
    glBindVertexArray(0);
